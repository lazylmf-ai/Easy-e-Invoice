export const MALAYSIAN_VALIDATION_RULES = [
    {
        code: 'MY-001',
        severity: 'error',
        field: 'supplier.tin',
...
export function validateInvoice(invoice, lines, org) {
    const results = [];
...
export function calculateValidationScore(results) {
    if (results.length === 0)
        return 100;
...
export const B2C_CONSOLIDATION_RULES = {
    ...
    // Validation function
    validateConsolidation(invoice, org, lineCount) {
        ...
        const condition = this.conditional[org.industryCode];
        if (condition) {
