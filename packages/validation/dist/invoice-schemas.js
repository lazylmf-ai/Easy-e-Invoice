...
const decimalString = z.string().regex(/^\d+(\.\d{1,2})?$/, 'Must be a valid decimal with up to 2 decimal places');
// Positive decimal string
const positiveDecimalString = z.string().regex(/^[1-9]\d*(\.\d{1,2})?$|^0\.\d*[1-9]\d*$/, 'Must be a positive decimal');
// Malaysian postcode validation
const malaysianPostcode = z.string().regex(/^\d{5}$/, 'Malaysian postcode must be 5 digits');
// Phone number validation (Malaysian format)
const malaysianPhone = z.string().regex(/^(\+?6?0?1[0-46-9]-*\d{7,8})|(\+?6?0?[4-9]-*\d{7,8})$/, 'Invalid Malaysian phone number format').optional();
// Email validation
...
export const invoiceSchema = z.object({
    ...
    consolidationPeriod: z.string().regex(/^\d{4}-\d{2}$/, 'Consolidation period must be in YYYY-MM format').optional(),
...
export const completeInvoiceSchema = z.object({
    ...
    const lineItemsSubtotal = data.lineItems.reduce((sum, line) => sum + parseFloat(line.lineTotal), 0);
    ...
    const lineItemsSst = data.lineItems.reduce((sum, line) => sum + parseFloat(line.sstAmount), 0);
...
export function validateInvoiceNumber(invoiceNumber, orgPrefix) {
...
export function calculateSstAmount(lineTotal, sstRate) {
    return Math.round((lineTotal * sstRate / 100) * 100) / 100;
}
export function calculateGrandTotal(subtotal, discount, sstAmount) {
