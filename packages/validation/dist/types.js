...
export const tinSchema = z.string().refine((tin) => /^[A-Z]\d{10}$|^\d{12}$/.test(tin), 'Malaysian TIN must be format C1234567890 or 123456789012');
// SST rate validation
export const sstRateSchema = z.number().refine((rate) => [0, 6].includes(rate), 'SST rate must be 0% or 6%');
// Currency validation
